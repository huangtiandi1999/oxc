...
impl Rule for NoDuplicateImports {
    ...
    fn run_once(&self, ctx: &LintContext) {
        ...
        let requested_modules = &module_record.requested_modules;

        let is_type_import = |source: &str, span: Span| -> bool {
            requested_modules
                .get(source)
                .and_then(|v| v.iter().find(|r| r.span == span))
        ...
        for entry in &module_record.import_entries {
            ...
            if previous_span != Some(span) {
                ...
                if let Some(existing) = import_map.get(source) {
                    if can_merge_imports((&import_type, entry.is_type), existing, self.allow_separate_type_imports) {
            ...
            let is_type = is_type_import(source, span);
            import_map.entry(source).or_default().push((import_type, span, ModuleType::Import, is_type));
        }
        ...
        if self.include_exports {
            for entry in &module_record.star_export_entries {
                ...
                let span = entry.span;
                let entry_is_type = entry.is_type;

                if entry.import_name.is_all_but_default() {
                    if let Some(existing) = import_map.get(source) {
                        if existing.iter().any(|(t, .., is_type)| {
                            // 允许值类型分开
                            if !matches!(t, ImportType::AllButDefault) {
                                return false;
                            }
                            if self.allow_separate_type_imports {
                                return entry_is_type == *is_type;
                            }
                            true
                        }) {
            ...
            for entry in &module_record.indirect_export_entries {
                // export { a } from 'foo'; import type { A } from 'foo'
                let Some(module_request) = &entry.module_request else {
                    continue;
                };
                let source = &module_request.name;
                ...
                if let Some(existing) = import_map.get(source) {
                    if entry.import_name == ExportImportName::All {
                        if existing.iter().any(|(t, .., is_type)| {
                            if !matches!(t, ImportType::Default | ImportType::Namespace) {
                                return false;
                            }
                            if self.allow_separate_type_imports {
                                return entry_is_type == *is_type;
                    ...
                    if existing.iter().any(|(t, _, module_type, is_type)| {
                        (matches!(
                            t,
...
fn can_merge_imports(
    (current_type, current_is_type): (&ImportType, bool),
    existing: &[(ImportType, Span, ModuleType, bool)],
    allow_separate_type_imports: bool,
) -> bool {
    let named = existing.iter().find(|(t, .., is_type)| {
        if allow_separate_type_imports {
            if current_is_type {
                return *is_type && matches!(t, ImportType::Named);
            } else {
                return !*is_type && matches!(t, ImportType::Named);
            }
        }
        matches!(t, ImportType::Named)
    });

    let default = existing.iter().find(|(t, .., is_type)| {
        if allow_separate_type_imports {
            if current_is_type {
                return *is_type && matches!(t, ImportType::Default);
            } else {
                return !*is_type && matches!(t, ImportType::Default);
            }
        }
    ...
    let namespace = existing.iter().find(|(t, .., is_type)| {
        if allow_separate_type_imports {
            if current_is_type {
                return *is_type && matches!(t, ImportType::Namespace);
            } else {
                return !*is_type && matches!(t, ImportType::Namespace);
            }
        }
        matches!(t, ImportType::Namespace)
    });
    ...
    match current_type {
        ImportType::Named => has_named || has_default,
        ImportType::Namespace => {
            if has_named && has_default {
                if let Some((_, named_span, ..)) = named {
...
fn test() {
    ...
    let fail = vec![
        ...
        //         export type * from 'foo';
        //     ",
        //     Some(serde_json::json!([{ "allowSeparateTypeImports": false, "includeExports": true }])),
        // ),
        // (
        //     "
        //         export type { a } from 'foo';
